...
const RocketGame = {
  ...
  renderGameContainer() {
    ...
    container.innerHTML = `
      ...
      <div class="rocket-arena" id="rocketArena" onclick="RocketGame.focusInput()">
        <!-- Focus catcher for keyboard events -->
        <input type="text" id="rocketKeyFocus" class="rocket-focus-input" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false" />
  ...
  focusInput() {
    const input = document.getElementById('rocketKeyFocus');
    if (input) {
      input.focus();
    }
  },
  ...
  bindKeyListeners() {
    ...
    // Direct event listener on focused hidden input and window fallback
    ...
    this._keyListener = processKeyEvent;

    window.addEventListener('keydown', this._keyListener, true);

    // Attach to input element directly as well
    setTimeout(() => {
      const input = document.getElementById('rocketKeyFocus');
      if (input) {
        input.addEventListener('input', (e) => {
          if (input.value) {
            const char = input.value.slice(-1).toLowerCase();
